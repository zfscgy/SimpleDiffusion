...
data_root = Path("/home/zf/projects/data")  # Change this to your preferred directory


class Mnist:
    @staticmethod
    def get(flatten_x: bool = False):
        """
        ...
        x_transforms = []
        if flatten_x:
            x_transforms.append(lambda x: x.view(28 * 28))

        mnist_train = _MNIST((data_root / "mnist").as_posix(),
                             transform=Compose([ToTensor()] + x_transforms), download=True)
...
class FashionMnist:
    @staticmethod
    def get(flatten_x: bool = False):
        x_transforms = [ToTensor()]
        if flatten_x:
            x_transforms.append(lambda x: x.view(3 * 28 * 28))

        fashion_train = _FashionMNIST((data_root / "fashion-mnist").as_posix(),
                             transform=Compose(x_transforms), download=True)
        fashion_test = _FashionMNIST((data_root / "fashion-mnist").as_posix(), train=False,
                            transform=Compose(x_transforms), download=True)
...
class Cifar10:
    @staticmethod
    def get(augumentation_x_train: bool = True, flatten_x: bool=False):
        """
        ...
        x_transforms = [ToTensor()]
        x_transforms_train = [ToTensor()]
        if augumentation_x_train:
            x_transforms_train.extend([RandomHorizontalFlip(p=0.5), RandomCrop(32, padding=4)])
        if flatten_x:
            x_transforms_train.append(lambda x: x.view(3 * 32 * 32))
            x_transforms.append(lambda x: x.view(3 * 32 * 32))

        cifar10_train = _CIFAR10((data_root / "cifar10").as_posix(), transform=Compose(x_transforms_train))
        cifar10_test = _CIFAR10((data_root / "cifar10").as_posix(), train=False, transform=Compose(x_transforms))
...
class Cifar100:
    ...
    def get(augumentation_x_train: bool = True, flatten_x: bool=False):
        ...
        cifar100_train = _CIFAR100((data_root / "cifar100").as_posix(), transform=Compose(x_transforms_train))
        cifar100_test = _CIFAR100((data_root / "cifar100").as_posix(), train=False, transform=Compose(x_transforms))
...
class DBPedia:
    ...
    @classmethod
    def get(cls, seq_len: int = 100) -> (Dataset, Dataset, Dataset):
        classes = cls.classes
        assert classes in [9, 219], "DBPedia only contains 9-class and 219-class versions"
        if DBPedia.train_set is None:
            DBPedia.train_set = pickle.load(open(data_root / f"dbpedia-{classes}/DBPedia_train.pkl", "rb"))
            DBPedia.validation_set = pickle.load(open(data_root / f"dbpedia-{classes}/DBPedia_val.pkl", "rb"))
            DBPedia.test_set = pickle.load(open(data_root / f"dbpedia-{classes}/DBPedia_test.pkl", "rb"))
            DBPedia.word2idx = pickle.load(open(data_root / f"dbpedia-{classes}/word_map.pkl", "rb"))
            DBPedia.label2idx = pickle.load(open(data_root / f"dbpedia-{classes}/label_map.pkl", "rb"))
...
class DBPedia9(DBPedia):
    classes = 9


class DBPedia219(DBPedia):
    classes = 219


if __name__ == '__main__':
    def mnist_test():
        train, test = Mnist.get(False)
        xs, ys = next(iter(DataLoader(train)))
        return xs, ys

    def fashion_mnist_test():
        train, test = FashionMnist.get(False)
        xs, ys = next(iter(DataLoader(train)))
        print(xs.shape, ys.shape)

    def cifar100_test():
        train, test = Cifar100.get(augumentation_x_train=True)
        xs, ys = next(iter(DataLoader(train)))
        print(xs.shape, ys.shape)
    ...
    def dbpedia_test():
        train, val, test = DBPedia9.get(10)
        data_iterator = iter(DataLoader(train))
        for i in range(10):
            xs, ys = next(data_iterator)
            print(xs, ys)
